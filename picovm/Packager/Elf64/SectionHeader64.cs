...
namespace picovm.Packager.Elf64
{
    public struct SectionHeader64
    {
        ...
        [Description("This member's value gives the byte offset from the beginning of the file to the first byte in the section.  One section type,S H T _ N O B I T Sdescribed below, occupies no space in the file, and its sh_offset member locates the conceptual placement in the file.")]
        public UInt64 SH_OFFSET;

        [Description("This member gives the section's size in bytes.  Unless the section type is SHT_NOBITS, the section occupiess h _ s i z ebytesin the file.  A section of type SHT_NOBITS may have a non-zero size, but it occupies no space in the file")]
        ...
        public UInt16 Write(Stream stream, HeaderIdentityClass EI_CLASS)
        {
            ...
            headerLength += stream.WriteXWord((UInt32)SH_FLAGS);
            headerLength += stream.WriteAddress64(SH_ADDR);
            headerLength += stream.WriteOffset64(SH_OFFSET);
            headerLength += stream.WriteXWord((UInt32)SH_SIZE);
            headerLength += stream.WriteWord((UInt32)SH_LINK);
            headerLength += stream.WriteWord((UInt32)SH_INFO);
            headerLength += stream.WriteXWord((UInt32)SH_ADDRALIGN);
            headerLength += stream.WriteXWord((UInt32)SH_ENTSIZE);
